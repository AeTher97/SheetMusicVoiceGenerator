...
def kodou(score,
          metadata={},
          file_name="kodou",
          path="D:/glosowe/package/tmp",
          dot_kodou=r"D:/glosowe/.kodou",
          outputs=["pdf"],
          view=True):
    ...
    if non_midi_formats: # write ly data
        ...
        lilypond = "F:/Program Files (x86)/LilyPond/usr/bin"
        print(lilypond)
        # write to ly file
        ...
        print("/".join((path_, file_name)))
        print(ly_path+"lilypond")
        lilypond_popenargs = ["lilypond", "-o", "/".join((path_, file_name)), compile_flags, ly_path]
        print(lilypond_popenargs)
